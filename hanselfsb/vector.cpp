...
// what is the need to create a slow functions, without protection huh ???
// well, I really should use the bit_vector_t object, but I still can't ... 
// no time but who knows ... .FD
...
// Returns:	N/A
// Notes:	N/A
void Vector::clearBit(int index)
{
	if (index <= size)
		array[index - 1] = 0;
};
...
// Notes:	N/A
Boolean Vector::retrieveBit(int index) const
{
...
// Returns:	N/A
// Notes:	N/A
void Vector::setValue(Boolean answer)
{
	valueKnown = TRUE;
	value = answer;
};
...
// Notes:	N/A
Boolean Vector::retrieveValue(void) const
{
	return value;
};
...
// Notes:	N/A
Boolean Vector::retrieveInquiredStatus(void) const
{
	return wasInquired;
};
...
// Notes:       N/A
Boolean Vector::retrieveKnown(void) const {
	return valueKnown;
...
void Vector::clearVector(void) {
	...
	}	// end clears each vector element
}; // Vector::clearVector
...
// Notes:       N/A
Vector Vector::operator &(/* IN */ Vector& secondVector) const {
	//Vector* thirdVector;
	//thirdVector = new Vector(size);
...
Boolean Vector::operator ==(/* IN */ Vector& secondVector) const {
	...
	for (int index = 0; index < size; index++) {
		...
		if (secondVector.array[index] != array[index]) {
			return FALSE;
		}	// end selfVector is not equal to secondVector
	}	// checks both vectors' elements
	return TRUE;
}; // Vector::operator ==
...
// Returns:     Result of operation
// Notes:       N/A
Boolean Vector::operator !=(/* IN */ const Vector& secondVector) const {
...
// Returns:     Result of operation
// Notes:       N/A
Boolean Vector::operator =(/* IN */ const Vector& secondVector) {

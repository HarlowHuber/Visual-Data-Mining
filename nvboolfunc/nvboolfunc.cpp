...
void nvboolfunc_t::set_input(const nvinput_t* const _input)
{
	...
	if (input)
	{
		...
		while (!input->eof())
		{
			...
			if (!safe_store(vect, r)) // store real element (vector)
			{
...
void nvboolfunc_t::little_sort(void)
{
	...
	if (!is3d)
	{
		...
		size_t i, j, nb = 0;
		bit_vector_t v0(size - 1, 0), v(size - 1);
		generic_inode_t cur;
		...
		for (lvl = 0; lvl < size; lvl++)
		{
			...
			if (disk_data[lvl].inode.generic_pointer) // checks if there is data for the current disk
			{
				...
				{
					p[j] = j;
					v[p[j]] = true; // biggest bit vector for disk
				}
	...
	if (disk_data)
	{
		...
		if (disk_data[i].inode.generic_pointer)
		{
			if (!is3d)
			{
				...
				while (cur && cur->bit_vector != vect) cur = cur->next;
				if (!cur && addit)
				{
		...
		else return store_inode(vect, value, is_fake);
	}
	return NULL;
}
